...
M0_c = np.zeros([1, H, W, 32], np.float32)              
M0_c = tf.Variable(M0_c)
M1_p = np.zeros([1, H, W//2, 32], np.float32)       
M1_p = tf.Variable(M1_p)
M1_c = np.zeros([1, H, W//2, 48], np.float32)       
M1_c = tf.Variable(M1_c)
M2_p = np.zeros([1, H, W//4, 48], np.float32)       
M2_p = tf.Variable(M2_p)
M2_c = np.zeros([1, H, W//4, 96], np.float32)       
M2_c = tf.Variable(M2_c)
M3_p = np.zeros([1, H, W//8, 96], np.float32)       
M3_p = tf.Variable(M3_p)
M3_c = np.zeros([1, H, W//8, 128], np.float32)       
M3_c = tf.Variable(M3_c)
M4_p = np.zeros([1, H, W//16, 128], np.float32)       
M4_p = tf.Variable(M4_p)
M4_c = np.zeros([1, H, W//16, 192], np.float32)      
M4_c = tf.Variable(M4_c)
M5_p = np.zeros([1, H, W//32, 192], np.float32)      
M5_p = tf.Variable(M5_p)
M5_c = np.zeros([1, H, W//32, 256], np.float32)      
M5_c = tf.Variable(M5_c)
M6_p = np.zeros([1, H, W//64, 256], np.float32)     
M6_p = tf.Variable(M6_p)
M6_c = np.zeros([1, H, W//64, 320], np.float32)     
M6_c = tf.Variable(M6_c)
M7_p = np.zeros([1, H, W//128, 320], np.float32)      
M7_p = tf.Variable(M7_p)
M7_c = np.zeros([1, H, W//128, 384], np.float32)      
...
for file in files: 
    ...
    for i in range(0, T):
      ...
      M0_c[:, t1, :, :].assign(net[:, 0, :, :])
      t0 = (t1-1)%H      
      net = tf.concat([tf.slice(M0_c, [0,t0,0,0], [-1,1,-1,-1]), tf.slice(M0_c, [0,t1,0,0], [-1,1,-1,-1])], axis=1)
      net = slim.pool(net, [s, s], stride=[s, s], pooling_type='MAX')
      M1_p[:, t1, :, :].assign(net[:, 0, :, :])
      t0 = (t1-2)%H
      net1 = tf.concat([tf.slice(M1_p, [0,t0,0,0], [-1,1,-1,-1]), tf.slice(M1_p, [0,t1,0,0], [-1,1,-1,-1])], axis=1)
      net = tf.nn.relu(conv(net1, w1, b1), name='ReLU')
      M1_c[:, t1, :, :].assign(net[:, 0, :, :])           
      net = tf.concat([tf.slice(M1_c, [0,t0,0,0], [-1,1,-1,-1]), tf.slice(M1_c, [0,t1,0,0], [-1,1,-1,-1])], axis=1)
 
      net = slim.pool(net, [s, s], stride=[s, s], pooling_type='MAX')
      M2_p[:, t1, :, :].assign(net[:, 0, :, :])
      t0 = (t1-4)%H
      net2 = tf.concat([tf.slice(M2_p, [0,t0,0,0], [-1,1,-1,-1]), tf.slice(M2_p, [0,t1,0,0], [-1,1,-1,-1])], axis=1)
      net = tf.nn.relu(conv(net2, w2, b2), name='ReLU')           
      M2_c[:, t1, :, :].assign(net[:, 0, :, :])     
      net = tf.concat([tf.slice(M2_c, [0,t0,0,0], [-1,1,-1,-1]), tf.slice(M2_c, [0,t1,0,0], [-1,1,-1,-1])], axis=1)
 
      net = slim.pool(net, [s, s], stride=[s, s], pooling_type='MAX')
      M3_p[:, t1, :, :].assign(net[:, 0, :, :])
      t0 = (t1-8)%H
      net3 = tf.concat([tf.slice(M3_p, [0,t0,0,0], [-1,1,-1,-1]), tf.slice(M3_p, [0,t1,0,0], [-1,1,-1,-1])], axis=1)
      net = tf.nn.relu(conv(net3, w3, b3), name='ReLU')           
      M3_c[:, t1, :, :].assign(net[:, 0, :, :])     
      net = tf.concat([tf.slice(M3_c, [0,t0,0,0], [-1,1,-1,-1]), tf.slice(M3_c, [0,t1,0,0], [-1,1,-1,-1])], axis=1)
 
      net = slim.pool(net, [s, s], stride=[s, s], pooling_type='MAX')
      M4_p[:, t1, :, :].assign(net[:, 0, :, :])
      t0 = (t1-16)%H
      net4 = tf.concat([tf.slice(M4_p, [0,t0,0,0], [-1,1,-1,-1]), tf.slice(M4_p, [0,t1,0,0], [-1,1,-1,-1])], axis=1)
      net = tf.nn.relu(conv(net4, w4, b4), name='ReLU')         
      M4_c[:, t1, :, :].assign(net[:, 0, :, :])     
      net = tf.concat([tf.slice(M4_c, [0,t0,0,0], [-1,1,-1,-1]), tf.slice(M4_c, [0,t1,0,0], [-1,1,-1,-1])], axis=1)
 
      net = slim.pool(net, [s, s], stride=[s, s], pooling_type='MAX')
      M5_p[:, t1, :, :].assign(net[:, 0, :, :])
      t0 = (t1-32)%H
      net5 = tf.concat([tf.slice(M5_p, [0,t0,0,0], [-1,1,-1,-1]), tf.slice(M5_p, [0,t1,0,0], [-1,1,-1,-1])], axis=1)
      net = tf.nn.relu(conv(net5, w5, b5), name='ReLU')            
      M5_c[:, t1, :, :].assign(net[:, 0, :, :])      
      net = tf.concat([tf.slice(M5_c, [0,t0,0,0], [-1,1,-1,-1]), tf.slice(M5_c, [0,t1,0,0], [-1,1,-1,-1])], axis=1)
 
      net = slim.pool(net, [s, s], stride=[s, s], pooling_type='MAX')
      M6_p[:, t1, :, :].assign(net[:, 0, :, :])
      t0 = (t1-64)%H
      net6 = tf.concat([tf.slice(M6_p, [0,t1,0,0], [-1,1,-1,-1]), tf.slice(M6_p, [0,t1,0,0], [-1,1,-1,-1])], axis=1)
      net = tf.nn.relu(conv(net6, w6, b6), name='ReLU')           
      M6_c[:, t1, :, :].assign(net[:, 0, :, :])     
      net = tf.concat([tf.slice(M6_c, [0,t0,0,0], [-1,1,-1,-1]), tf.slice(M6_c, [0,t1,0,0], [-1,1,-1,-1])], axis=1)
       
      net = slim.pool(net, [s, s], stride=[s, s], pooling_type='MAX')
      M7_p[:, t1, :, :].assign(net[:, 0, :, :])
      t0 = (t1-128)%H
      net7 = tf.concat([tf.slice(M7_p, [0,t0,0,0], [-1,1,-1,-1]), tf.slice(M7_p, [0,t1,0,0], [-1,1,-1,-1])], axis=1)
      net = tf.nn.relu(conv(net7, w7, b7), name='ReLU')           
      M7_c[:, t1, :, :].assign(net[:, 0, :, :])     
      net = tf.concat([tf.slice(M7_c, [0,t0,0,0], [-1,1,-1,-1]), tf.slice(M7_c, [0,t1,0,0], [-1,1,-1,-1])], axis=1)
      net = slim.pool(net, [s, s], stride=[s, s], pooling_type='MAX')
